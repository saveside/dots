...
# Kernel parameters
if grep -E "zswap.enabled=1|zswap.max_pool_percent=6|zswap.compressor=zstd|transparent_hugepage=always|intel-pstate=active|mitigations=off|skew_tick=1|nvidia_drm.modeset=1" /etc/kernel/cmdline; then
	...
	if [[ -f /etc/kernel/cmdline ]]; then
		...
		echo -n " zswap.enabled=1 zswap.max_pool_percent=6 zswap.compressor=zstd transparent_hugepage=always intel-pstate=active mitigations=off skew_tick=1 nvidia_drm.modeset=1" >>/etc/kernel/cmdline
